...
#include "codeflinger-powerpc/PPCAssemblerProxy.h"
...
#define CONTEXT_LOAD(REG, FIELD) \
    LWZ(REG, GGL_OFFSETOF(FIELD), mBuilderContext.Rctx)

#define CONTEXT_STORE(REG, FIELD) \
    STW(REG, GGL_OFFSETOF(FIELD), mBuilderContext.Rctx)
...
public:
    ...
    {
    public:
        Spill(RegisterFile& regFile, PPCAssemblerInterface& gen, uint32_t reglist)
            : mRegFile(regFile), mGen(gen), mRegList(reglist), mCount(0)
        {
            if (reglist) {
                int count = 0;
		uint32_t savereg = reglist;
                while (reglist) {
                    count++;
                    reglist &= ~(1 << (31 - __builtin_clz(reglist)));
                }
		// If used, this should never generate more than two
		// registers to save.  We automatically have two slots
		// on the 96 bytes stack to use as part of the ABI call,
		// so use them.
		if (count > 2) {
		    LOGE("Spill Overflow");
		    count = 2;
		...
		while(--count >= 0) {
                    int svreg;
                    svreg = 31 - __builtin_clz(savereg);
                    savereg &= ~(1 << svreg);
		    mGen.STW(svreg, 8 + count*4, mGen.stkp);
		}
        ...
        ~Spill() {
            if (mRegList) {
		int savereg = mRegList;
		while(--mCount >= 0) {
                    int svreg;
                    svreg = 31 - __builtin_clz(savereg);
                    savereg &= ~(1 << svreg);
		    mGen.LWZ(svreg, 8 + mCount*4, mGen.stkp);
		}
...
class GGLAssembler : public PPCAssemblerProxy, public RegisterAllocator
{
public:

                    GGLAssembler(PPCAssemblerInterface* target);
