...
#define CONTEXT_LOAD(REG, FIELD) \
    LW(REG, GGL_OFFSETOF(FIELD), mBuilderContext.Rctx)

#define CONTEXT_STORE(REG, FIELD) \
    SW(REG, GGL_OFFSETOF(FIELD), mBuilderContext.Rctx)
...
public:
    ...
    {
    public:
        Spill(RegisterFile& regFile, MIPSAssemblerInterface& gen, uint32_t reglist)
		...
		// If used, this should never generate more than four
		// registers to save.  We automatically have four slots
		// on the stack to use as part of the ABI call, so use them.
		if (count > 4) {
		    LOGE("Spill Overflow");
		    count = 4;
		}
		...
		while(--count >= 0) {
                    ...
                    savereg &= ~(1 << svreg);
		    mGen.SW(svreg, count*4, mGen.stkp);
		}
		...
		while(--mCount >= 0) {
                    ...
                    svreg = 31 - __builtin_clz(savereg);
                    savereg &= ~(1 << svreg);
		    mGen.LW(svreg, mCount*4, mGen.stkp);
		}
...
class GGLAssembler : public MIPSAssemblerProxy, public RegisterAllocator
{
public:

                    GGLAssembler(MIPSAssemblerInterface* target);
