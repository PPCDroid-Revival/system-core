                        ...
                        Scratch& regs)
{
   if (mInfo[component].fog) {
        ...
        LBU(fogColor.reg, GGL_OFFSETOF(state.fog.color[component]),
        ...
        SRA(at, factor.reg, 31);
        NOR(at, zero, at);
        AND(factor.reg, factor.reg, at);

	// Compare factor.reg > 0x10000, if so clamp at 0x10000
	int tmp = regs.obtain();
	LUI(at, 1);	// 0x10000
	SLT(tmp, factor.reg, at);
	MOVZ(factor.reg, at, tmp);
                        ...
                        Scratch& regs)
{
   if (!mInfo[component].blend)
    ...
    if (temp.l) {
        ...
        SRL(fragment.reg, temp.reg, temp.l);
        ...
        Scratch& scratches)
{
    ...
    case GGL_DST_COLOR:
        factor.s = fb.s;
        SRL(at, fb.reg, fb.s-1);
        ADDU(factor.reg, fb.reg, at);
        break;
    case GGL_ONE_MINUS_SRC_COLOR:
    case GGL_SRC_COLOR:
        factor.s = fragment.s;
        SRL(at, fragment.reg, fragment.s-1);
        ADDU(factor.reg, fragment.reg, at);
        break;
    case GGL_ONE_MINUS_SRC_ALPHA:
    case GGL_SRC_ALPHA:
        factor.s = src_alpha.s;
        SRL(at, src_alpha.reg, src_alpha.s-1);
        ADDU(factor.reg, src_alpha.reg, at);
        break;
    case GGL_ONE_MINUS_DST_ALPHA:
    case GGL_DST_ALPHA:
    ...
    if (factor.s > 8) {
        SRL(factor.reg, factor.reg, factor.s-8);
        factor.s = 8;
    }
}
        ...
        const integer_t& fb)
{
    ...
    if (shift>0) {
    	SRL(at, fragment.reg, shift);
        SUBU(diff.reg, at, fb.reg);
    }
    else if (shift<0) {
    	SLL(at, fragment.reg, -shift);
        SUBU(diff.reg, at, fb.reg);
    }
    else {
        SUBU(diff.reg, fragment.reg, fb.reg);
    }
        ...
        const integer_t& fb)
{
    ...
    if (shift>0) {
        ...
        SUBU(diff.reg, fb.reg, fragment.reg);
    }
        ...
        const integer_t& dst, const integer_t& src)
{
    ...
    if (!shift) {
        ADDU(d.reg, src.reg, dst.reg);
    } else {
        SLL(at, dst.reg, shift);
        ADDU(d.reg, src.reg, at);
    }
